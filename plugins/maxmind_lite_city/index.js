...
/**
 * MaxMind Lite City IP list plugin
 * Downloads and processes GeoLite2-City-CSV data from MaxMind
 */
class MaxmindLiteCityPlugin extends BasePlugin {
    /**
    ...
    constructor(options = {}) {
        super({
            name: 'maxmind_lite_city',
            version: '1.0.0',
            description: 'Downloads and processes GeoLite2-City-CSV data from MaxMind',
    ...
    /**
     * Download MaxMind GeoLite2-City-CSV zip file
     * @returns {Promise<void>}
     * @private
     */
    async _downloadMaxmind() {
        this.logger.info('Downloading MaxMind GeoLite2-City-CSV');
        const writer = fs.createWriteStream(this.zipFile);
        
        return this.retryWithBackoff(async () => {
            const response = await axios({
                method: 'get',
                url: `https://download.maxmind.com/app/geoip_download?edition_id=GeoLite2-City-CSV&license_key=${this.apiKey}&suffix=zip`,
                responseType: 'stream',
     ...
     * @returns {Promise<Object>} City data indexed by geoname_id
     * @private
     */
    async _loadCities() {
        this.logger.info('Loading city location data');
        ...
        return new Promise((resolve, reject) => {
            fs.createReadStream(filename)
                ...
                .on('end', () => {
                    this.logger.debug({ cityCount: Object.keys(data).length }, 'City data loaded');
                    resolve(data);
                })
                .on('error', reject);
        });
     ...
     * @param {Object} cities - City data indexed by geoname_id
     * @returns {Promise<string>} Plugin name
     * @private
     */
    async _processRanges(cities) {
        this.logger.info('Processing IP ranges');
    ...
    async load() {
        ...
        this._interval = setInterval(() => {
            this.logger.debug('Still working on maxmind_lite_city');
        }, 5000).unref();

        try {
            await this._downloadMaxmind();
            await this._extract();
            const cities = await this._loadCities();
            return await this._processRanges(cities);
        } finally {
            if (this._interval) {
...
module.exports = MaxmindLiteCityPlugin;
